...
//SELECIONAR DADOS DA PUBLICAÇÃO QUE ESTA SENDO ALTERADA
  $id = $_GET["id"];
  ...
  $DataSelecionada;
     ...
     include 'abreConexao.php';
                    ...
                    $sql = "SELECT * FROM tb_publicacao WHERE ID = '$id' AND ID_GRUPO_FK = '$idGrupoA'";
              
                $result = $conexao->query($sql);

              //alimenta Select 
                while($row = $result->fetch_assoc()) {
                      $tituloSelecionado = $row["TITULO"];
                      $idProjetoSelecionado = $row["ID_PROJETO_FK"];
                      $TipoSelecionado = $row["TIPO"];
                      $DocenteSelecionado = $row["ID_DOCENTE_FK"];
                      $linhaSelecionada = $row["ID_LINHA_FK"];
                      $RefereciaSelecionada = $row["REFERENCIAS"];
                      $DataSelecionada = $row["DATA_PUBLICACAO"];
...
<bR><bR><br>
<div>
<form id="formulario" method="post" enctype="multipart/form-data"" action="AtualizaPublicacaoSql.php">
  <div class="container">
    <?php
    ...
    <hr>
      ...
      <input type="hidden"  <?php echo 'value="'. $_GET["id"].'"' ?>  name="idPub" > 
   <input type="text" <?php echo 'value="'.$tituloSelecionado.'"' ?>  maxlength="60" name="Titulo" id="Titulo" required>

       <label for="text"><b>Projeto de Pesquisa:</b></label><br>
   <select class="form-control" id="cbProj" onchange="pegaDocente()" name="cbProj">
      <option value="0">NÃO POSSUI....</option>
        ...
        <?php  
              ...
              //alimenta Select 
                while($row = $result->fetch_assoc()) {
                      if($row['ID'] == $idProjetoSelecionado){
...
<br>
 ...
 <label for="text"><b>Tipo:</b></label><br>
   <select class="form-control" id="TipoPublic" name="TipoPublic" required>     
   		...
   		if($TipoSelecionado == 'Livro'){
   		 echo'<option value="'.$TipoSelecionado.'" selected>'.$TipoSelecionado.'</option>'; 	
   		}else{
        echo'<option value="Livro">Livro</option>';
    	}

    	if($TipoSelecionado == 'Capítulo de Livro'){
   		 echo'<option value="'.$TipoSelecionado.'" selected>'.$TipoSelecionado.'</option>'; 	
   		}else{
        echo'<option value="Capítulo de Livro">Capítulo de Livro</option>';
    	}

    	if($TipoSelecionado == 'Anais de Congresso'){
   		 echo'<option value="'.$TipoSelecionado.'" selected>'.$TipoSelecionado.'</option>'; 	
   		}else{
        echo'<option value="Anais de Congresso">Anais de Congresso</option>';
    	}

    	if($TipoSelecionado == 'Periódicos'){
   		 echo'<option value="'.$TipoSelecionado.'" selected>'.$TipoSelecionado.'</option>'; 	
   		}else{
        echo'<option value="Periódicos">Periódicos</option>';
        }
...
<br>
  ...
  <!-- <label for="text"><b>Docente Do Projeto:</b></label>
   <input type="text"  maxlength="60" name="Textdocente" id="Textdocente" required>
      -->
        ...
        <label for="text"><b>Docente Do Grupo:</b></label>   
 ...
 <label for="text"><b>Linha de Pesquisa:</b></label>   
      ...
      <?php
          $Descricao = $RefereciaSelecionada;
     ...
     <br>
    <textarea name="Referencias" id="Referencias"  required rows="08" ><?php echo $Descricao ?></textarea>
    <br><br>
    ...
    <input class='col-4' type='date' name='data' id='DSaida' required value='<?php echo date("$ano-$mes-$dia"); ?>'>
     
<br><br>



      <button type="submit" onclick="HabilitaSelects()" class="registerbtn">Alterar Publicação</button>
