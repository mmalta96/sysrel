...
<body>
	...
	<div class="container">
		...
		<?php 
            ...
            $ano = $_GET["ano"];
		?>
		<h3>Relatório de Publicações do Grupo no ano de <?php echo $ano; ?></h3>

        <table class="table table-striped">
            <thead>
                <tr>
                    <th><h3>Publicações</h3></th>
                </tr>
            </thead>
            	...
            	<?php 
                    ...
                    $sql = "SELECT PU.TITULO, PU.TIPO, DATE_FORMAT(PU.DATA_PUBLICACAO,'%d/%m/%Y') AS DATA, PU.REFERENCIAS,
						LP.NOME AS LINHA, D.NOME AS DOCENTE

						FROM tb_publicacao PU 
						INNER JOIN tb_linha_pesquisa LP 
						ON LP.ID = PU.ID_LINHA_FK
						AND PU.DATA_PUBLICACAO BETWEEN '".$ano."-01-01 00:00:00' AND '".$ano."-12-31 23:59:59'

						INNER JOIN tb_docentes D 
						on D.ID = PU.ID_DOCENTE_FK

						AND PU.ID_GRUPO_FK = ".$_SESSION["idGrupoAtual"];
            		...
            		while ($row = $result->fetch_assoc()) {
            			...
            			echo "<h5>Título: ".$row["TITULO"]."</h5>";
                        echo "<h5>Tipo: ".$row["TIPO"]."</h5>";
                        echo "<h5>Data da Publicação: ".$row["DATA"]."</h5>";
                        echo "<h5>Linha de Pesquisa: ".$row["LINHA"]."</h5>";
                        echo "<h5>Docente Responsável: ".$row["DOCENTE"]."</h5>";
                        echo "<h5>Referências: ".$row["REFERENCIAS"]."</h5>";
